...
(function (root, factory) {
  ...
  var TOPICS = {
    general: { name: 'General', field: null, glyph: '\u2732',
      lens: 'read wide, so whatever is loudest in your life right now is what these cards will attach themselves to',
      close: 'A general reading is broad on purpose. What fits will fit obviously, and what does not is not a message you have failed to decode. Take the part that landed and leave the rest on the table.' },
    love: { name: 'Love and Romance', field: 'love', glyph: '\u2661',
      ...
      close: 'Nothing here obliges anybody. A reading can put words to a pattern in a relationship, and only the people inside it decide what happens next. If something here landed, the useful next step is usually a conversation rather than a decision.' },
    finance: { name: 'Finance', field: 'fin', glyph: '\u25C8',
      lens: 'read through material life: work, money, security, and the difference between building something and keeping it standing',
      close: 'This is not financial advice and cannot be. Treat anything it seems to point at as a question worth checking against real numbers, never an instruction to act on. Money questions reward slow answers.' }
  };

  var DEPTHS = {
    light: { name: 'Light', counts: [3, 6], blurb: 'Three or six cards, read quickly: the shape of the thing, where its weight is sitting, and one place to put your attention today.' },
    divine: { name: 'Divine', counts: [6, 9], blurb: 'Six or nine cards in full paragraphs, each one read against the cards beside it, ending on the single move the draw keeps pointing at.' },
    omni: { name: 'Omni', counts: [9, 11], blurb: 'The long study. Nine or eleven cards, every one read against every other, seven passages, ending in a pattern named clearly enough that you can check it against what actually happens.' }
  };
  ...
  function linkClause(a, b) {
    return a.rev === b.rev
      ? (a.rev ? 'Both fell reversed, so the pair reads as one held breath described twice rather than as two separate problems. '
               : 'Both stand upright, so they reinforce one another rather than pulling in different directions. ')
      : 'One is reversed and one is not, and that is where the tension in this reading sits. ';
  }
  function pairLine(a, b, topic) {
    return linkClause(a, b) + sent(a) + ' Set against that: ' + sent(b);
  }
  function arcLine(drawn) {
    ...
    return 'The reading opens on ' + label(f) + ' in the position of ' + low(f.pos) +
      ' and closes on ' + label(l) + ' at ' + low(l.pos) + '. Read the first as the ground you are standing on while you ask, and the last as what that ground is turning into.';
  }
  function suitLine(t, drawn) {
    var dom = dominantSuit(t), gone = absentSuits(t), bits = [];
    if (dom && dom.count > 1 && dom.count * 3 >= drawn.length) {
      bits.push(dom.count + ' of the ' + drawn.length + ' cards are ' + dom.suit.name +
        ', which puts the weight of this reading on ' + dom.suit.arena + '.');
    } else if (dom && dom.count > 1) {
      bits.push(dom.count + ' of the ' + drawn.length + ' cards are ' + dom.suit.name +
        ', the largest single group but a long way from a majority, so the reading leans towards ' +
        dom.suit.arena.split(',')[0] + ' without settling there.');
    } else if (t.major >= Math.ceil(drawn.length / 2)) {
      bits.push('The suits are scattered and the Major Arcana carry the reading instead, which the tradition reads as a matter larger than the daily one, and slower to turn.');
    } else {
      bits.push('No suit dominates, so the reading is spread across several parts of your life rather than gathered into one. That is a wider draw rather than a weaker one.');
    }
    if (gone.length === 1) {
      bits.push('Nothing from ' + gone[0].name + ' appears at all, so ' + gone[0].arena.split(',')[0] + ' is either settled or being kept outside the question. It is worth knowing which of the two it is.');
    } else if (gone.length > 1 && gone.length < 4) {
      bits.push('Two of the four suits are missing entirely, which narrows the reading more than a full spread usually allows: useful on a focused question, thin on a broad one.');
    }
    return bits.join(' ');
  }
  function orientLine(t, drawn) {
    var up = drawn.length - t.rev;
    if (t.rev === 0) return 'Every card fell upright. The tradition reads an all upright draw as a situation with no hidden brake in it: what you can see is the whole mechanism, which is easier to work with than it sounds.';
    if (t.rev === drawn.length) return 'Every card fell reversed. That is unusual, and it is read as one obstruction repeated at every level rather than as many separate problems. One thing to shift rather than five.';
    return up + ' upright against ' + t.rev + ' reversed. Reversed cards are not bad cards. They mark where the current is running backwards, or where something has not been let out yet, and both of those are things that move.';
  }
  function majorLine(t, drawn) {
    if (!t.major) return 'No Major Arcana appear. The tradition takes that as a matter still in your hands, worked out through daily choices rather than by anything larger than you.';
    if (t.major === 1) return 'One Major Arcanum stands in the draw, and the tradition treats it as the hinge: the minor cards around it describe how that one larger movement is being lived out day to day.';
    return t.major + ' Major Arcana in a draw of ' + drawn.length + '. That density is read as a season rather than an episode. Seasons are not usually argued out of, and they do end.';
  }
  function repeatLine(t) {
    ...
    return cap(named.join('; ')) + '. A repeated number is the tradition\u2019s loudest signal: the same lesson offered in more than one part of your life at once, which usually means it can be learned in the easier place first and carried to the harder one.';
  }
  function courtLine(t) {
    if (!t.courts) return '';
    if (t.courts === 1) return 'One court card appears. Courts are usually read as people, or as the part of you that behaves like one, so expect a specific figure in this rather than a mood.';
    return t.courts + ' court cards appear, which the tradition reads as other people being genuinely involved rather than as a private matter. Whatever gets decided here will be decided with somebody.';
  }
  function elementLine(t) {
    ...
    var opp = e[pair];
    return 'By element the draw runs ' + hi + ' heavy' + (opp ? ', with ' + pair + ' present as a counterweight. The tradition reads that pairing as a temperature problem rather than a direction problem: the right move at the wrong heat.'
      : ', with nothing of ' + pair + ' to answer it, so the reading tends to overshoot in that direction. Borrowing the missing element on purpose is the usual correction.');
  }
  ...
  /* ---------- the three readings ---------- */
  /* Light is the short reading, and short is not the same as thin. It used to
     be three sentences, which is a summary rather than a reading: it named the
     cards, said what arrived, and stopped before it was any use to somebody who
     came here worried about something real. It names the whole draw now, says
     what the first and last cards are doing to each other, gives one line of
     shape, and closes on something that can be acted on today. Divine and Omni
     are longer because they say more, not because this one is being held back
     to make them look bigger. */
  function light(drawn, topic) {
    var T = TOPICS[topic], t = tally(drawn), dom = dominantSuit(t);
    var f = drawn[0], m = drawn[Math.floor(drawn.length / 2)], l = drawn[drawn.length - 1];
    var laid = drawn.map(function (d) { return label(d) + ' at ' + low(d.pos); }).join(', ');
    var focus = topic === 'general'
      ? (dom && dom.count > 1
          ? 'If you take one thing from this, take ' + dom.suit.arena.split(',')[0] + '. That is where the draw is densest, and where a small change would show up first.'
          : 'If you take one thing from this, take ' + label(m) + ' at ' + low(m.pos) + '. ' + sent(m))
      : 'In this question: ' + cap(strip(topicClause(l, topic))) + '.';
    return [
      { tag: 'CALCULATED', text: 'Laid out: ' + laid + '. ' +
        (drawn.length - t.rev) + ' upright, ' + t.rev + ' reversed.' },
      { tag: 'TRADITIONAL', text: pairLine(f, l, topic) },
      { tag: 'TRADITIONAL', text: [(dom && dom.count > 1) ? suitLine(t, drawn) : majorLine(t, drawn),
        orientLine(t, drawn)].filter(Boolean).join(' ') },
      { tag: 'POSSIBILITY', text: focus + ' ' + T.close }
    ];
  }

  function divine(drawn, topic) {
    var t = tally(drawn), T = TOPICS[topic], out = [];
    out.push({ tag: 'CALCULATED', text: arcLine(drawn) + ' ' + drawn.length + ' cards in all, ' +
      (drawn.length - t.rev) + ' upright and ' + t.rev + ' reversed. They are ' + T.lens + '.' });
    ...
    out.push({ tag: 'POSSIBILITY', text: 'Taken together, the draw points at one move rather than several, and it is this one. ' +
      sent(last) + ' ' + (repeatLine(t) || courtLine(t) || '') + ' ' + T.close });
    return out;
  }

  function omni(drawn, topic) {
    var t = tally(drawn), T = TOPICS[topic], out = [], dom = dominantSuit(t);
    out.push({ tag: 'CALCULATED', text: artN(drawn.length) + ' ' + drawn.length + ' card draw. They are ' + T.lens + '. ' +
    ...
    out.push({ tag: 'TRADITIONAL', text: 'Read in sequence, the cards hand on to one another rather than standing apart, and the handovers are where the reading is. ' +
    ...
    out.push({ tag: 'TRADITIONAL', text: orientLine(t, drawn) + ' ' + majorLine(t, drawn) + ' ' +
      (t.rev ? 'Where a card is reversed, the tradition does not flip its meaning into its opposite. It reads the same force turned inward, delayed, or refused, which is why reversed cards so often describe something you already know and have not yet acted on. Naming it is most of the work.' : '') });
    ...
    var f = drawn[0], m = drawn[Math.floor(drawn.length / 2)], l = drawn[drawn.length - 1];
    out.push({ tag: 'SYNTHESIS', text: 'Woven together, the reading turns on three of its cards, and this next part is assembled here rather than quoted from the tradition. ' + label(f) +
      ' sets the ground. ' + sent(f) + ' ' + label(m) + ' is the pivot. ' + sent(m) +
      ' ' + label(l) + ' is where it lands. ' + sent(l) + ' ' + linkClause(f, l) });
    ...
    out.push({ tag: 'POSSIBILITY', text: 'If the pattern holds, the next stretch looks like this. ' +
      ...
      'This is a prediction in the only sense a spread can offer one: a pattern named clearly enough that you can hold it up against what actually happens, and discard it honestly if it does not hold. A pattern you can see is one you can do something about, and that is the whole of what a spread is for. ' + T.close });

    return out;
  }

  function read(o) {
    var count = o.count, topic = o.topic || 'general', depth = o.depth || 'light';
    var drawn = o.drawn || draw(count, o.seed || 1);
    var passages = (depth === 'light' ? light(drawn, topic)
      : depth === 'divine' ? divine(drawn, topic)
      : omni(drawn, topic)).map(function (p) {
  ...
  return {
    ...
    INTRO: {
      what: 'Tarot is a deck of seventy eight images with five centuries of commentary attached. The cards know nothing about you, and that is the useful part: they hold still while you look, and they show a situation from angles you would not have picked for yourself.',
      how: 'A reading here is drawn at random and kept with the number that produced it, so the same shuffle can be laid out again exactly. It is woven rather than listed: every card is read against the ones beside it, in the order they fell and the direction they landed.',
      caution: 'Nothing here has been shown to predict anything by any means outside the tradition itself. Read it as a mirror with a vocabulary: it can put words to something you already half knew, and it cannot tell you what happens next. Keep your own record of whether it was ever useful to you, and trust that record over this one.'
    }
  };
}));
