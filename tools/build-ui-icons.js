...
/* The same lens turned on its side, for a pair of circles stacked rather than
   set beside each other. Same construction as lensAt with the axes swapped:
   two circles of radius r whose centres are r apart, so the overlap is
   r*sqrt(3)/2 wide either side of the centre. */
function lensH(cx, cy, r) {
  const h = (r * Math.sqrt(3) / 2);
  return 'M' + (cx - h).toFixed(2) + ' ' + cy + 'A' + r + ' ' + r + ' 0 0 1 ' + (cx + h).toFixed(2) + ' ' + cy +
         'A' + r + ' ' + r + ' 0 0 1 ' + (cx - h).toFixed(2) + ' ' + cy + 'Z';
}
...
const ICONS = {
  ...
  /* DREAMS. The one screen where the lens is the literal subject rather than a
     figure for it. Two wholes, waking above and sleeping below, and the dream
     is exactly the part they have in common: made by the sleeper, addressed to
     the one who wakes. Stacked rather than side by side, because the pair here
     is one person over a night and not two people over a table, which is what
     the horizontal arrangement means everywhere else in this set. */
  dream:
    '<circle cx="12" cy="8.9" r="6.2" ' + S + '/>' +
    '<circle cx="12" cy="15.1" r="6.2" ' + S + '/>' +
    '<path d="' + lensH(12, 12, 6.2) + '" ' + F + '/>',
