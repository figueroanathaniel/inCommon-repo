...
/* THE PUBLISH DIRECTORY IS WRITTEN HERE TOO, for exactly the reason the two
   _redirects are.

   netlify.toml named the bundle folder in three places and carried a comment
   asking whoever cut a new folder to bump it. It drifted the first time
   somebody forgot: the builder moved to v6.3 and publish stayed on v6.2, so a
   git connected deploy served the previous bundle while a dragged repository
   served the current one. The same repository, two different sites, and
   nothing anywhere reporting it, because both deploys succeed.

   A comment asking a human to keep two constants in step is the thing that
   drifts. The folder is named once, in BUNDLE at the top of this file, and
   every other place that needs it is generated from there or reads it from
   here. netlify.toml is static TOML that the host parses, so it cannot derive
   anything: the only way it can be right is if this writes it.

   The file is rewritten rather than patched in place, because a regex over
   somebody else's edit is how a generator corrupts a file it does not own. */
function writeNetlifyToml() {
  const dir = path.relative(repo, BUNDLE).split(path.sep).join('/');
  const out = [
    '# GENERATED by tools/build-bundle.js. Do not edit by hand: publish names the',
    '# bundle folder, and that folder is renamed at every version bump.',
    '#',
    '# The repo root is not the site.',
    '#',
    '# There is no index.html at this level and there is not meant to be: deploy/',
    '# holds one folder per shipped build, and the site is one of those folders.',
    '# Point a host at the repo without this file and it serves a directory that has',
    '# no page in it, which is a working deploy of nothing and reads as a broken',
    '# link.',
    '#',
    '# There is no build step. The bundle in that folder is already built, by',
    '# tools/build-bundle.js, run here rather than at the host.',
    '#',
    '# How the site is served is NOT restated here. The two rules that matter, every',
    '# path resolving to the app and must-revalidate on the three files a redeploy',
    '# has to reach, live beside the bundle in ' + dir + '/_redirects and',
    '# ' + dir + '/_headers, so a folder dragged onto app.netlify.com/drop is served',
    '# exactly like a deploy from the repo. Two files declaring the same rule is two',
  ...
  const at = path.join(repo, 'netlify.toml');
  const before = fs.existsSync(at) ? fs.readFileSync(at, 'utf8') : '';
  fs.writeFileSync(at, out, 'utf8');
  const wasDir = (/publish\s*=\s*"([^"]+)"/.exec(before) || [])[1];
  return { dir: dir, moved: wasDir && wasDir !== dir ? wasDir : null };
}
const netlify = writeNetlifyToml();
...
console.log('  netlify.toml:    publish = ' + netlify.dir +
  (netlify.moved ? '   (was ' + netlify.moved + ', which would have served the previous bundle)' : ''));
